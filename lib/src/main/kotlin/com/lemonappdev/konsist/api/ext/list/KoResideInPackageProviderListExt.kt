 ...
 */
fun <T : KoResideInPackageProvider> List<T>.withPackage(vararg names: String): List<T> = filter {
    when {
 ...
 */
fun <T : KoResideInPackageProvider> List<T>.withoutPackage(vararg names: String): List<T> = filter {
    when {
        names.isEmpty() -> (it as? KoPackageProvider)?.packagee == null
